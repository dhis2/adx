<?xml version="1.0"?>
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:message="http://www.SDMX.org/resources/SDMXML/schemas/v2_0/message" xmlns:structure="http://www.SDMX.org/resources/SDMXML/schemas/v2_0/structure" xmlns:generic="http://www.SDMX.org/resources/SDMXML/schemas/v2_0/generic" version="1.0">
	...
	<xsl:template match="message:GenericData">
		...
		<xsl:variable name="FreqDim" select="$KeyFamStruct/structure:Components/structure:Dimension[@isFrequencyDimension='true']/@conceptRef"/>
...
ARR+<xsl:value-of select="count(../../generic:SeriesKey/generic:Value)+2"/>+<xsl:for-each select="../../generic:SeriesKey/generic:Value"><xsl:value-of select="@value"/>:</xsl:for-each><xsl:value-of select="$TimePeriod"/>'<xsl:for-each select="generic:Value[@concept!='OBS_STATUS' and @concept!='OBS_CONF' and @concept!='OBS_PRE_BREAK']">
	...
	<xsl:template match="generic:Value" mode="output-att">
		...
		<xsl:choose>
			<xsl:when test="not($KeyFamStruct/structure:Components/structure:Attribute[@conceptRef = $AttConcept]/@codelist)">
...
CDV+<xsl:value-of select="@value"/>'</xsl:otherwise>
	...
	<xsl:template name="isSequential">
		...
		<xsl:choose>
			...
			<xsl:when test="$freq-val='W'">
				<xsl:variable name="test_two">
					<xsl:variable name="year" select="substring($one,1,4)"/>
					<xsl:variable name="week" select="substring($one,5,2)"/>
					<xsl:choose>
						<xsl:when test="$week=52"><xsl:value-of select="concat($year+1,'01')"/></xsl:when>
						<xsl:otherwise>
							<xsl:variable name="newWeek" select="$week+1"/>
							<xsl:variable name="newWeekText">
								<xsl:choose>
									<xsl:when test="string-length($newWeek)=1"><xsl:value-of select="concat('0',$newWeek)"/></xsl:when>
									<xsl:otherwise><xsl:value-of select="$newWeek"/></xsl:otherwise>
								</xsl:choose>
							</xsl:variable>
							<xsl:value-of select="concat($year,$newWeekText)"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:variable>
				<xsl:choose>
	...
	<xsl:template name="getTimeFormat">
		...
		<xsl:choose>
			...
			<xsl:when test="$freq-val='W'">
				<xsl:choose>
					<xsl:when test="$one = $two">:616</xsl:when>
					<xsl:otherwise>:716</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:when test="$freq-val='M'">
				<xsl:choose>
	...
	<xsl:template match="generic:Obs" mode="output-time-period">
		...
		<xsl:choose>
			...
			<xsl:when test="$freq-val='W'">
				<xsl:value-of select="substring($time,1,4)"/>
				<xsl:choose>
					<xsl:when test="contains($time, '-W')">
						<xsl:if test="2>string-length(substring($time,7,2))">0</xsl:if><xsl:value-of select="substring($time,7,2)"/>
					</xsl:when>
				</xsl:choose>
			</xsl:when>
			<xsl:when test="$freq-val='M'">
