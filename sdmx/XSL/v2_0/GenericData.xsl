...
<xsl:template match="/">
     ...
     <body>
          <xsl:variable name="KeyFamName">
               ...
               <xsl:choose>              	
                    <xsl:when test="$KeyFam/structure:Name[1]">
                         <xsl:value-of select="$KeyFam/structure:Name[1]"/>
                    </xsl:when>
                    <xsl:otherwise>
					...
					<xsl:otherwise>
               <xsl:choose>              	
                    <xsl:when test="$KeyFam/structure:Name[@xml:lang=$lang]">
                         <xsl:value-of select="$KeyFam/structure:Name[@xml:lang=$lang]"/>
                    </xsl:when>
                    <xsl:when test="$KeyFam/structure:Name[1]">
                         <xsl:value-of select="$KeyFam/structure:Name[1]"/>
                    </xsl:when>                    
               ...
               <h2 class="tableTitle" onclick="hideTable('{$TableTitle}')" onmouseover="window.status='Expand or collapse series data table.';" onmouseout="window.status=''">Series Key:
						<xsl:choose>
	                       ...
	                       <xsl:variable name="DimName">
	                       	<xsl:apply-templates mode="GetName" select="$KeyFam/structure:Components/structure:Dimension[@conceptRef=$Dim]"/>
	                       </xsl:variable>
	                       <xsl:variable name="CodeValue" select="@value"/>
	                       <xsl:variable name="CodeDesc">
	                       	<xsl:apply-templates mode="GetValueDescription" select="$KeyFam/structure:Components/structure:Dimension[@conceptRef=$Dim]">
                                                                      ...
                                                                      <xsl:for-each select="ancestor-or-self::*/generic:Attributes/generic:Value">
                                                                           <tr>
																			                      ...
																			                      <xsl:variable name="AttName">
																			                      	<xsl:apply-templates mode="GetName" select="$KeyFam/structure:Components/structure:Attribute[@conceptRef=$Att]"/>
																			                      </xsl:variable>
																			                      <xsl:variable name="CodeValue" select="@value"/>
																			                      <xsl:variable name="CodeDesc">
																			                      	<xsl:apply-templates mode="GetValueDescription" select="$KeyFam/structure:Components/structure:Attribute[@conceptRef=$Att]">
																			                      		<xsl:with-param name="value" select="$CodeValue"/>
																								</xsl:apply-templates>
																				                      </xsl:variable>
                                             ...
                                             <tbody>
                                                  <tr>
                                                       ...
                                                       <xsl:for-each select="generic:Obs/generic:Attributes/generic:Value/@concept[not(../../../preceding-sibling::*/generic:Attributes/generic:Value/@concept = .)]">
                                                            ...
                                                            <th class="serField"><xsl:value-of select="$AttName"/></th>
                                                       </xsl:for-each>
                                                  </tr>
