									...
									<xsl:copy>
										<xsl:for-each select="*[local-name()='Series' and *[local-name()='Obs']]">
											<xsl:variable name="SeriesKey">
												<!-- Get Series Key Dimensions, sort them alphabetically by name, and pass {name}"="{value} into variable -->
												<xsl:for-each select="@*[local-name() = $Dimensions/@conceptRef]">
											...
											<xsl:copy>
												<xsl:copy-of select="@*"/>
												<xsl:for-each select="//*[local-name()='Series' and not(child::*)]">
													<xsl:variable name="DocAtts" select="@*"/>
													<xsl:variable name="TestSeriesKey">
													<!-- Get Series Key Dimensions, sort them alphabetically by name, and pass {name}"="{value} into variable -->
													<xsl:for-each select="@*[local-name() = $Dimensions/@conceptRef]">
														<xsl:sort select="local-name(.)"/>
														<xsl:value-of select="local-name(.)"/>
													...
													<xsl:if test="$SeriesKey=$TestSeriesKey">
														<xsl:copy-of select="$DocAtts"/>
													</xsl:if>
												</xsl:for-each>
